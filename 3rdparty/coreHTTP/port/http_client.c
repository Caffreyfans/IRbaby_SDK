...
uint8_t http_url_parse(HTTPClient *client)
{
    ...
    int parser_status = http_parser_parse_url(client->url, strlen(client->url), 0, &purl);
    if (parser_status != 0)
    {
        return CLIENT_FAIL;
    }
    ...
    {
        client->schema = malloc_str(client->url + purl.field_data[UF_SCHEMA].off, purl.field_data[UF_SCHEMA].len);
    ...
    {
        client->host = malloc_str(client->url + purl.field_data[UF_HOST].off, purl.field_data[UF_HOST].len);
    }

    if (purl.field_data[UF_PORT].len)
    {
        client->port = malloc_str(client->url + purl.field_data[UF_PORT].off, purl.field_data[UF_PORT].len);
    }

    if (purl.field_data[UF_PATH].len)
    {
        client->path = malloc_str(client->url + purl.field_data[UF_PATH].off, purl.field_data[UF_PATH].len);
    }
    LOG("client->schema = %s\n", client->schema);
    LOG("client->host = %s\n", client->host);
    LOG("client->port = %s\n", client->port);
...
uint8_t http_client_send(HTTPClient *client)
{
    ...
    HTTPStatus_t status = HTTPClient_Send(&client->interface,
                                          &client->header,
                                          client->payload,
                                          client->payload_len,
                                          &client->response,
...
uint8_t http_client_init(HTTPClient *client)
{
    ...
    if (http_url_parse(client) != CLIENT_OK)
        goto err;
    LOG("http_url_parse ok\n");
    if (open_transport(client) != CLIENT_OK)
        goto err;
    ...
    int success = (((header_buffer = (uint8_t *)malloc(HEADER_MAX_LEN)) != NULL) &&
    ...
    HTTPStatus_t status = HTTPClient_InitializeRequestHeaders(&client->header,
                                                              &requestInfo);
